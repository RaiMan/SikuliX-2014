...
//TODO why? JXGrabKey unresolved: pthread
	...
	private static boolean buildVision(String srcjar) {
      ...
      //log(lvl, "JDK: not found - set JAVA_HOME to a valid JDK");
			//buildComplete = false;
			log1(lvl, "buildVision: JDK: not found - using the bundled include files");
		...
		if (!new File(inclUsr, inclLib).exists() && !new File(inclUsrLocal, inclLib).exists()) {
			log1(lvl, "buildVision: opencv-include: not found - using the bundled include files");
		...
		inclLib = "tesseract";
		if (!new File(inclUsr, inclLib).exists() && !new File(inclUsrLocal, inclLib).exists()) {
			log1(lvl, "buildVision: tesseract-include: not found - using the bundled include files");
		...
		if (rl != null) {
			...
			try {
				...
				log1(-1, "buildVision: cannot write %s", cmdFile);
				return false;
			}
